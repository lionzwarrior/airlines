...
class DatabaseWrapper:
    ...
    def post_ticket(self, customer_name, flight_code, date):
        cursor = self.connection.cursor(dictionary=True)
        sql = f"INSERT INTO `ticket`(`customer_name`, `flight_id`) SELECT '{customer_name}', f.id FROM flight f INNER JOIN flight_code fc ON f.flight_code_id = fc.id WHERE fc.flight_code = '{flight_code}' and f.date = '{date}';"
        try:
            cursor.execute(sql)
    ...
    def get_ticket(self, customer_name, date):
        ...
        try:
            ...
            hours, remainder = divmod(temp_start_time, 3600)
            minutes, seconds = divmod(remainder, 60)
            temp_finish_time = result["end_time"].total_seconds()
            hours2, remainder2 = divmod(temp_finish_time, 3600)
            minutes2, seconds2 = divmod(remainder2, 60)
            result['start_time'] = '{:02}:{:02}:{:02}'.format(int(hours), int(minutes), int(seconds))
            result['end_time'] = '{:02}:{:02}:{:02}'.format(int(hours2), int(minutes2), int(seconds2))
            ...
            if result:
                ...
                response = {
                    "status": "not_found",
                    "message": f"ticket with customer_name {customer_name} not found"
                }
        except Exception as e:
            response = {
                "status": str(e)
